...
<div class="container py-2">
    <div class="row mb-4 align-items-center">
        <div class="col-12 col-md-6">
            ...
            <p class="text-secondary mb-0">
                {{ $unreadNotificationCount ?? 0 }} unread notifications
            </p>
        </div>
        <div class="col-12 col-md-6 text-md-end mt-3 mt-md-0">
            @if(isset($unreadNotificationCount) && $unreadNotificationCount > 0)
                <form method="POST" action="{{ route('staff.notifications.markAllRead') }}" class="d-inline">
                    @csrf
    ...
    <ul class="nav nav-tabs border-bottom mb-4">
        <li class="nav-item">
            <a class="nav-link fw-semibold {{ request('filter') === null ? 'active text-primary' : 'text-secondary' }}" href="{{ route('staff.notifications.index') }}">
                All Notifications
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link fw-semibold {{ request('filter') === 'unread' ? 'active text-primary' : 'text-secondary' }}" href="{{ route('staff.notifications.index', ['filter' => 'unread']) }}">
                Unread
                @if(isset($unreadNotificationCount) && $unreadNotificationCount > 0)
        ...
        <li class="nav-item">
            <a class="nav-link fw-semibold {{ request('filter') === 'system' ? 'active text-primary' : 'text-secondary' }}" href="{{ route('staff.notifications.index', ['filter' => 'system']) }}">
                System
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link fw-semibold {{ request('filter') === 'email' ? 'active text-primary' : 'text-secondary' }}" href="{{ route('staff.notifications.index', ['filter' => 'email']) }}">
                Email
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link fw-semibold {{ request('filter') === 'sms' ? 'active text-primary' : 'text-secondary' }}" href="{{ route('staff.notifications.index', ['filter' => 'sms']) }}">
                SMS
            </a>
        </li>
    </ul>
                ...
                <div class="list-group">
                    @foreach($notifications as $notification)
                        ...
                        <div class="list-group-item list-group-item-action p-3 mb-2 border rounded shadow-sm clickable-item position-relative"
                            ...
                            <form id="mark-read-form-{{ $notification->id }}" action="{{ route('staff.notifications.markRead', $notification) }}" method="POST" class="d-none">
                                @csrf
                            </form>
                                    ...
                                    <div class="d-flex justify-content-between align-items-center mt-2 border-top pt-2">
                                        <div>
                                            @if($notification->order_id && $notification->order)
                                                <a href="{{ route('staff.orders.show', $notification->order_id) }}" class="badge bg-primary-subtle text-primary text-decoration-none border border-primary-subtle px-2.5 py-1.5 fs-8">
