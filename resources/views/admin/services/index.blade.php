@extends('layouts.admin')

@section('content')
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-4 border-bottom">
    <h1 class="h2 fw-bold text-dark">Services Directory</h1>
    <a href="{{ route('admin.services.create') }}" class="btn btn-primary fw-semibold">
        <i class="fa-solid fa-plus me-2"></i>Add New Service
    </a>
</div>

<!-- Search and Filter Form Card -->
<div class="card border-0 shadow-sm rounded-4 mb-4">
    <div class="card-body p-4">
        <form method="GET" action="{{ route('admin.services.index') }}" class="row g-3">
            <!-- Search field -->
            <div class="col-12 col-md-6">
                <label for="search" class="form-label fw-semibold text-muted small">Search Service Name</label>
                <div class="input-group">
                    <span class="input-group-text bg-light border-end-0 text-muted"><i class="fa-solid fa-magnifying-glass"></i></span>
                    <input type="text" name="search" id="search" class="form-control bg-light border-start-0" placeholder="Type query..." value="{{ request('search') }}">
                </div>
            </div>

            <!-- Status filter -->
            <div class="col-12 col-sm-6 col-md-3">
                <label for="status" class="form-label fw-semibold text-muted small">Filter by Status</label>
                <select name="status" id="status" class="form-select bg-light">
                    <option value="">All Statuses</option>
                    <option value="active" {{ request('status') === 'active' ? 'selected' : '' }}>Active</option>
                    <option value="inactive" {{ request('status') === 'inactive' ? 'selected' : '' }}>Inactive</option>
                </select>
            </div>

            <!-- Filter Buttons -->
            <div class="col-12 col-sm-6 col-md-3 d-flex align-items-end gap-2">
                <button type="submit" class="btn btn-primary w-100 fw-bold">Filter</button>
                <a href="{{ route('admin.services.index') }}" class="btn btn-outline-secondary w-100 fw-bold">Clear</a>
            </div>
        </form>
    </div>
</div>

<!-- Services List Card -->
<div class="card border-0 shadow-sm rounded-4">
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                <thead class="table-light">
                    <tr>
                        <th class="border-0 px-4">#</th>
                        <th class="border-0">Service Name</th>
                        <th class="border-0">Slug</th>
                        <th class="border-0">Price/kg</th>
                        <th class="border-0">Price/item</th>
                        <th class="border-0">Status</th>
                        <th class="border-0 px-4 text-end">Actions</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($services as $service)
                        <tr>
                            <td class="px-4 text-muted">{{ $service->id }}</td>
                            <td>
                                <div class="fw-semibold text-dark">{{ $service->name }}</div>
                                @if($service->description)
                                    <div class="small text-muted text-truncate">{{ Str::limit($service->description, 50) }}</div>
                                @endif
                            </td>
                            <td><code class="text-lowercase text-muted">{{ $service->slug }}</code></td>
                            <td class="fw-medium">${{ number_format($service->price_per_kg, 2) }}</td>
                            <td class="fw-medium">${{ number_format($service->price_per_item, 2) }}</td>
                            <td>
                                @if($service->is_active)
                                    <span class="badge bg-success">Active</span>
                                @else
                                    <span class="badge bg-secondary">Inactive</span>
                                @endif
                            </td>
                            <td class="px-4 text-end">
                                <div class="d-inline-flex gap-2">
                                    <!-- Toggle Status Button -->
                                    <form method="POST" action="{{ route('admin.services.toggleStatus', $service->id) }}" class="m-0">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="btn btn-sm btn-outline-secondary" title="Toggle Status">
                                            @if($service->is_active)
                                                <i class="fa-solid fa-toggle-on text-success"></i>
                                            @else
                                                <i class="fa-solid fa-toggle-off text-muted"></i>
                                            @endif
                                        </button>
                                    </form>

                                    <!-- Edit Button -->
                                    <a href="{{ route('admin.services.edit', $service->id) }}" class="btn btn-sm btn-outline-primary" title="Edit">
                                        <i class="fa-solid fa-pen"></i>
                                    </a>

                                    <!-- Delete Button -->
                                    <form method="POST" action="{{ route('admin.services.destroy', $service->id) }}" class="m-0 delete-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-outline-danger" title="Delete">
                                            <i class="fa-solid fa-trash-can"></i>
                                        </button>
                                    </form>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="text-center py-5 text-muted">
                                <i class="fa-solid fa-bell-slash fs-2 mb-2 d-block text-secondary"></i>
                                No services found in directory.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    </div>
</div>
@endsection

@section('scripts')
<script>
    document.addEventListener('DOMContentLoaded', function () {
        // Confirmation dialog for delete actions
        const deleteForms = document.querySelectorAll('.delete-form');
        deleteForms.forEach(function (form) {
            form.addEventListener('submit', function (event) {
                event.preventDefault();
                if (confirm('Are you sure you want to delete this service? It cannot be deleted if associated with any order items.')) {
                    form.submit();
                }
            });
        });
    });
</script>
@endsection
